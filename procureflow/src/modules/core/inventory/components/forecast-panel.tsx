    ...
    <div className="space-y-4 rounded-card border border-pf-border bg-pf-bg-secondary p-5">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="bg-pf-accent/10 hover:bg-pf-accent/20 inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium text-pf-accent transition-colors disabled:opacity-50"
        >
          ...
          {forecast.projected.map((value, i) => (
            <div key={monthLabels[i]} className="flex-1 text-center">
              <div className="mx-auto mb-1 w-full max-w-[40px]">
                <div
                  className="bg-pf-accent/60 rounded-t transition-all"
                  style={{
                    height: `${Math.max((value / maxProjected) * 60, 4)}px`,
                  }}
                />
              </div>
              <p className="text-[10px] text-pf-text-muted">{monthLabels[i]}</p>
      ...
      {/* AI Forecast Result */}
      {aiResult && (
        <div className="border-pf-accent/20 bg-pf-accent/5 space-y-2 rounded-lg border p-3">
          <div className="flex items-center gap-2">
